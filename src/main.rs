...
fn calculator(val_1: f32, val_2: f32, opr: &str) -> Result<f32, &str> {
    match opr {
        "+" => Ok(val_1 + val_2),
        "-" => Ok(val_1 - val_2),
        "*" => Ok(val_1 * val_2),
...
fn main() {
    ...
    let input_1: f32 = input_1.trim().parse().expect("Please enter a number!");
    
    eprint!("Enter the second value: ");
    io::stdin()
        .read_line(&mut input_2)
        .expect("Failed to read the second value.");
    let input_2: f32 = input_2.trim().parse().expect("Please enter a number!");
